    ...
    <div className="relative mx-auto w-full max-w-5xl overflow-hidden py-8 [perspective:1200px]">
      <div className="relative h-[360px] w-full">
        {certificates.map((item, index) => {
          ...
          const courseLinks = (Array.isArray(item.courseLinks) ? item.courseLinks : []).filter(Boolean)
          ...
          return (
            ...
            >
              ...
              <div className="mt-4 overflow-hidden rounded-2xl border border-line bg-bg/50">
                {item.previewImage ? (
                  <Image
                    src={item.previewImage}
                    alt={item.title ? `${item.title} certificate` : "Certificate preview"}
                    width={960}
                    height={420}
                    className="h-44 w-full object-cover"
                  />
                ) : (
                  <div className="grid h-44 place-items-center text-xs text-muted">Preview image coming soon</div>
                )}
              </div>
              {item.credentialUrl ? (
                <a
              ...
              {courseLinks.length ? (
                <div className="mt-4">
                  <p className="text-[11px] uppercase tracking-[0.18em] text-muted">Courses</p>
                  <div className="mt-2 flex flex-wrap gap-2">
                    {courseLinks.map((url, urlIndex) => (
                      <a
                        key={`${url}-${urlIndex}`}
                        href={url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="rounded-full border border-line px-3 py-1 text-xs font-semibold text-text transition hover:border-accent hover:text-accent"
                      >
                        Course {urlIndex + 1}
                      </a>
                    ))}
